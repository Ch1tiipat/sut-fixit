...
const MONTH_OPTIONS = ['มกราคม 2569', 'กุมภาพันธ์ 2569', 'มีนาคม 2569', 'ตุลาคม 2568', 'พฤศจิกายน 2568', 'ธันวาคม 2568'];

export default function TechHistoryScreen() {
    const [selectedMonth, setSelectedMonth] = useState('มีนาคม 2569');
    const [modalVisible, setModalVisible] = useState(false);
    const [historyTasks, setHistoryTasks] = useState<any[]>([]);
    const [loading, setLoading] = useState(true);

    // ✅ เพิ่ม State จัดการการแจ้งเตือน
    ...
    useEffect(() => {
        ...
        unsubTasks = onSnapshot(qTasks, (snapshot) => {
            ...
            const completedData = data.filter(d => d.status === "เสร็จสมบูรณ์" || d.status === "เสร็จสิ้น" || d.status === "Approved");
            completedData.sort((a: any, b: any) => new Date(b.closedAt || b.createdAt).getTime() - new Date(a.closedAt || a.createdAt).getTime());
            ...
            <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
                {loading ? (
                    ...
                    filteredTasks.map((task) => (
                        <TouchableOpacity 
                            key={task.id} style={styles.historyCard} activeOpacity={0.7}
                            onPress={() => router.push({ pathname: '/(technician)/history-detail', params: { id: task.id } })} 
                        >
                            <View style={styles.cardLeft}>
                                <View style={styles.successIconBox}><Ionicons name="checkmark-done" size={20} color="#10B981" /></View>
                                <View style={{ flex: 1 }}>
                                    ...
                                    <Text style={styles.historyDate}>ซ่อมเสร็จเมื่อ: {formatThaiDate(task.closedAt || task.createdAt)}</Text>
                                    
                                    {(task.rating || task.isReviewed) && (
                                        <View style={styles.reviewBox}>
                                            <View style={styles.reviewScoreRow}>
                                                <Ionicons name="star" size={14} color="#F59E0B" />
                                                <Text style={styles.reviewScoreText}>{task.rating?.quality || task.qualityRating || 5}.0 คะแนน</Text>
                                            </View>
                            ...
                            <View style={styles.cardRight}>
                                <View style={styles.statusBadge}><Text style={styles.statusText}>เสร็จสิ้น</Text></View>
...
const styles = StyleSheet.create({
    ...
    pageHeader: { paddingHorizontal: 20, paddingTop: 24, paddingBottom: 10 },
    pageTitle: { fontSize: 22, fontWeight: '800', color: '#111827' },
    monthFilter: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#FFF', marginHorizontal: 20, paddingHorizontal: 16, paddingVertical: 12, borderRadius: 12, marginBottom: 20, borderWidth: 1, borderColor: '#E5E7EB' },
    monthFilterText: { fontSize: 14, fontWeight: '700', color: '#4B5563' },
    scrollContent: { paddingBottom: 30 },
    historyCard: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', backgroundColor: '#FFFFFF', marginHorizontal: 20, marginBottom: 12, borderRadius: 16, padding: 16, borderWidth: 1, borderColor: '#F3F4F6', elevation: 1 },
    cardLeft: { flexDirection: 'row', alignItems: 'flex-start', flex: 1, marginRight: 10 },
    cardRight: { flexDirection: 'row', alignItems: 'center', alignSelf: 'center' },
    successIconBox: { width: 40, height: 40, borderRadius: 20, backgroundColor: '#D1FAE5', justifyContent: 'center', alignItems: 'center', marginRight: 14 },
    historyTitle: { fontSize: 15, fontWeight: '700', color: '#111827', marginBottom: 2 },
    historySubtitle: { fontSize: 13, color: '#6B7280', marginBottom: 4 },
    historyDate: { fontSize: 12, color: '#9CA3AF' },
    statusBadge: { backgroundColor: '#F0FDF4', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 8 },
    statusText: { color: '#10B981', fontSize: 12, fontWeight: '700' },
    reviewBox: { marginTop: 10, backgroundColor: '#FFFBEB', padding: 10, borderRadius: 8, borderWidth: 1, borderColor: '#FEF3C7' },
    reviewScoreRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 4 },
    reviewScoreText: { fontSize: 12, fontWeight: '700', color: '#D97706', marginLeft: 4 },
    reviewCommentText: { fontSize: 12, color: '#D97706', fontStyle: 'italic' },
