...
// ✅ 1. นำเข้า updateDoc และ onSnapshot เพิ่มเติม
...
export default function AdminSettingsScreen() {
  ...
  // ✅ 2. เปลี่ยนมาใช้ onSnapshot เพื่อดึงข้อมูลแบบ Real-time
  useEffect(() => {
    const auth = getAuth();
    ...
    if (user) {
      ...
      unsubscribe = onSnapshot(docRef, (docSnap) => {
        if (docSnap.exists()) {
          ...
          setAdminData(data);
          // ✅ ดึงสถานะการแจ้งเตือนจาก DB มาตั้งค่าที่ปุ่มสวิตช์
  ...
  // ✅ 3. ผูกปุ่มสวิตช์เข้ากับ Firebase (อัปเดตข้อมูลทันทีที่กด)
  const toggleSwitch = async (value: boolean) => {
    setIsNotificationEnabled(value); // เปลี่ยนในหน้าจอก่อนเพื่อให้รู้สึกลื่นไหล
    try {
      const auth = getAuth();
      const user = auth.currentUser;
      if (user) {
        // อัปเดตสถานะ pushEnabled ลง Firebase
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.card}>
          <View style={styles.cardRow}>
            ...
            {/* ✅ เรียกใช้ฟังก์ชัน toggleSwitch ที่สร้างใหม่ */}
            <Switch
...
const styles = StyleSheet.create({
  ...
  appSubtitle: { fontSize: 12, fontWeight: '700', color: '#EF4444', marginTop: 2 },
  ...
  avatarInner: { width: 56, height: 56, borderRadius: 28, backgroundColor: '#FFEDD5', justifyContent: 'flex-end', alignItems: 'center', overflow: 'hidden' },
  ...
  logoutBtn: { backgroundColor: '#FFF1F2', marginHorizontal: 20, marginTop: 40, paddingVertical: 16, borderRadius: 16, alignItems: 'center', justifyContent: 'center' },
