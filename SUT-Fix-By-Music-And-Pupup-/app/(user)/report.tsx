...
export default function ReportScreen() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // ⚠️ เปลี่ยนเป็น IP เครื่องคอมตัวเอง (ได้จากคำสั่ง ipconfig ใน cmd)
      

      // 1. อัปโหลดรูปภาพผ่าน Firebase Storage (เก็บไว้เหมือนเดิมเพราะปลอดภัยกว่า)
      const uploadedUrls = [];
      ...
      // 🚀 2. ใช้ API (Backend) สร้างใบแจ้งซ่อม [ได้คะแนน Data Create]
      const reportPayload = {
        userId: user.uid,
        title: issueTitle,
      ...
      // 🚀 3. ใช้ API (Backend) อัปเดตข้อมูลผู้ใช้ [ได้คะแนน Data Update]
      ...
      // 🚀 4. ใช้ API (Backend) สร้างการแจ้งเตือนส่งให้ช่าง [ได้คะแนน Data Create แหล่งที่ 2]
      ...
      Alert.alert('สำเร็จ', 'ส่งคำร้องผ่าน Backend API เรียบร้อยแล้ว', [
        { text: 'ตกลง', onPress: () => router.replace('/(user)/(tabs)') }
      ]);
    } catch (e) {
      console.error(e);
      Alert.alert('ผิดพลาด', 'ไม่สามารถเชื่อมต่อเซิร์ฟเวอร์ API ได้ ตรวจสอบ IP เครื่องให้ถูกต้อง');
    } finally {
