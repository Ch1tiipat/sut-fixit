...
แอปพลิเคชันมือถือ (Android) สำหรับนักศึกษาผู้พักอาศัย ช่างซ่อมบำรุง และผู้ดูแลระบบ เพื่อแจ้งซ่อม ติดตามสถานะ และบริหารงานซ่อมบำรุงหอพักแบบ Real-time 
รายวิชา 1101103 โครงงานการพัฒนาโปรแกรมประยุกต์ด้วยภาษาสคริปต์ ภาคการศึกษาที่ 3 ปีการศึกษา 2568 · สาขาวิชาเทคโนโลยีดิจิทัล SUT
...
SUT FixIt แก้ปัญหากระบวนการแจ้งซ่อมแบบเดิมที่อาศัยกระดาษหรือ Line/Facebook ซึ่งทำให้ข้อมูลตกหล่น ไม่มีระบบติดตาม และช่างหาห้องไม่พบ แอปนี้ทำหน้าที่เป็น **ศูนย์กลาง** ให้นักศึกษาแจ้งปัญหาพร้อมรูปถ่ายและพิกัด GPS ช่างรับงานพร้อมนำทาง และ Admin ดู Dashboard สถิติเพื่อบริหารทรัพยากรได้อย่างมีประสิทธิภาพ

---

## 🏗 สถาปัตยกรรมระบบ (System Architecture)

เพื่อประสิทธิภาพและการจัดการที่ดี โปรเจกต์นี้ถูกแบ่งออกเป็น 2 ส่วนหลัก:

1.  **Frontend (Mobile App - Repository นี้):** พัฒนาด้วย React Native (Expo) ทำหน้าที่เป็น UI สำหรับผู้ใช้งานทั้ง 3 กลุ่ม สื่อสารกับ Database และ API
2.  **Backend (API Server):** พัฒนาด้วย Node.js (Express) สำหรับจัดการ Logic บางส่วน แจ้งเตือน หรือจัดการข้อมูลที่ต้องใช้ Security สูง (แยก Repository: `sut-fixit-api` นำไป Deploy บน Render.com)
...
### 1. Clone โปรเจกต์
```bash
git clone [https://github.com/Kittipat050871/sut-fixit-app-2.git](https://github.com/Kittipat050871/sut-fixit-app-2.git)
cd sut-fixit-app-2
```
...
### 3. ตั้งค่า API URL (สำหรับการเรียกใช้ Backend)
แก้ไขไฟล์ `constants/api.ts` เพื่อชี้ไปยัง Backend API:
```typescript
// ไฟล์ constants/api.ts
// สำหรับรันเครื่องตัวเอง (แก้ IP ให้ตรงเครื่อง)
// export const API_URL = "[http://192.168.1.](http://192.168.1.)XXX:3000/api"; 

// สำหรับใช้งานจริง (เชื่อมไปยัง Render.com)
export const API_URL = "[https://sut-fixit-api.onrender.com/api](https://sut-fixit-api.onrender.com/api)"; // แก้เป็น URL ที่ได้จาก Render
```

### 4. ตั้งค่าไฟล์ Environment (Firebase)
สร้างไฟล์ `.env` ที่ root:
```bash
cp .env.example .env
```
กรอก Firebase config:
```env
...
### 1. สร้างโปรเจกต์
ไปที่ **Firebase Console** → **Add project** ตั้งชื่อ (เช่น `sut-fixit`) → Create

### 2. เพิ่มแอป Android
คลิกไอคอน Android → กรอก Package name: `com.sut.fixit`
ดาวน์โหลด `google-services.json` → วางที่ `android/app/google-services.json`

### 3. เปิดใช้งาน Services ที่จำเป็น

| Service | วิธีเปิดใช้ | ทำอะไรในแอป |
| :--- | :--- | :--- |
...
สร้าง API Key → เปิดใช้ **Maps SDK for Android**
ใส่ใน `app.json`:
```json
{
  "expo": {
...
│
├── constants/                    # ค่าคงที่ (สีธีม, หมวดหมู่, รายชื่อหอพัก, API_URL)
...
*   **ห้าม commit** ไฟล์ `.env`, `google-services.json` และ `serviceAccountKey.json` ขึ้น Git เด็ดขาด (มีใน `.gitignore` แล้ว)
...
*   **การอัปเดต API:** อย่าลืมเปลี่ยน `API_URL` ใน `constants/api.ts` เมื่อมีการนำ Backend ขึ้น Render.com เรียบร้อยแล้ว
```